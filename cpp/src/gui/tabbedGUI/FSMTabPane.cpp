...
void FSMTabPane::openFSM(Fsm * fsm) {
    ...
    //-- Connect FSMSceneView with Scene to forward signal mouseOutsideOfScene
    this->connect(sceneView,SIGNAL(mouseLeftScene()),fsmScene,SLOT(manageMouseLeftScene()));
    this->connect(sceneView,SIGNAL(mouseEnteredScene()),fsmScene,SLOT(manageMouseEnteredScene()));

    //-- Install an event filter on the scene to catch if mouse is outside of Scene
    fsmScene->installEventFilter(sceneView);
