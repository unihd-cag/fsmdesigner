...
QVariant StateItem::itemChange(GraphicsItemChange change,
	...
	if (change == QGraphicsItem::ItemSelectedChange) {
		...
		for (int i = 0; i < this->incomingTransitions.size(); i++) {
            ...
            Transline * tr = this->incomingTransitions.at(i);

            //List of all connected transitions
            QList<Transline*> connectedTransitions;
            connectedTransitions.append(tr);

            //Check if there are trackpoints
            QGraphicsItem * currentItem = tr->getStartItem();
            ...
            if(currentItem != nullptr)
            {
                ...
                double transAt = ceil(((double)connectedTransitions.size())/2);

                //odd numbers of transition
                if(connectedTransitions.size()%2 != 0)
                    transAt -= 1;

                tr = connectedTransitions.at(transAt);
            }

            Trans* trans = (Trans*)tr->getModel();
            if(trans != nullptr)
            {
                QGraphicsItem* textItem = this->scene()->itemAt(trans->getTextPosition().first,trans->getTextPosition().second);
                if (textItem != NULL) {
                    qreal textx = tr->getEndItem()->x() -(tr->getEndItem()->x() - tr->getStartItem()->x())/2;
                    qreal texty = tr->getEndItem()->y() -(tr->getEndItem()->y() - tr->getStartItem()->y())/2;
        ...
        for (int i = 0; i < this->outgoingTransitions.size(); i++) {
            ...
            if(currentItem != nullptr)
            {
                ...
                {
                    //Get next Transline
                    connectedTransitions.append(dynamic_cast<TrackpointItem*> (currentItem)->getNextTransline());
                    //Go to previous trackpoint if there is one
                    currentItem = dynamic_cast<TrackpointItem*> (currentItem)->getNextTransline()->getEndItem();
                }
