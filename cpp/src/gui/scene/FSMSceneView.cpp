...
FSMSceneView::FSMSceneView(Scene* scene, QWidget* parent) :
    ...
    this->connect(reloadVerilogButton->actions().first(),SIGNAL(triggered()),SLOT(prepareGenerateVerilog()));

    // Normal Generate
    QAction * reloadVerilogAction = new QAction("Generate to...",reloadVerilogButton);
    reloadVerilogButton->addAction(reloadVerilogAction);
    this->connect(reloadVerilogAction,SIGNAL(triggered()),SLOT(prepareGenerateVerilog()));
...
void FSMSceneView::wheelEvent(QWheelEvent *event) {
    ...
    {
        //Current scrolling value
        int value = this->horizontalScrollBar()->value();

        //Calculate wheel step
        double numDegrees = event->delta() / 8.0;
        double numSteps = numDegrees / 15;

        //186 = QT default value for scrolling for one wheel step
        value -= (numSteps*186);
...
//	}
    ...
    if (ke->key() ==  Qt::Key_Control && (this->getRelatedScene()->getPlaceMode()==FSMDesigner::STATE ||  this->getRelatedScene()->getPlaceMode()==FSMDesigner::TRANS)) {
...
void FSMSceneView::prepareGenerateVerilog() {
    ...
    //Check if scene has items -> if not -> don't generate
    if(this->getRelatedScene()->items().size()==0)
        return;

    //If it fails, don't generate verilog to avoid system crashes
    if(!verifyTable())
        return;
    ...
    //Ask for new path if action text == "Generate to..."
    if(action->text() == "Generate to...")
    {
        ...
        //-- Save path
        QString path = QFileDialog::getSaveFileName(this,"Choose a Verilog File to save to",location.path().append(defaultName),"Verilog Files (*.v)");
        if (path.length()==0)
            return;

        //Ensure that the last two chars of FileName is ".v"
        if (!path.endsWith(".v"))
            path.append(".v");
...
//            return;
    ...
    if (path.size()==0) {
        ...
        //-- Save path
        path = QFileDialog::getSaveFileName(this,"Choose a Verilog File to save to",location.path().append(defaultName),"Verilog Files (*.v)");
        if (path.length()==0)
            return;
...
void FSMSceneView::saveProject() {
    ...
    //-- If Project has a Save Location, save back, if not save as
...
bool FSMSceneView::verifyTable() {
    ...
    dialog.setWindowTitle("FSM Verification");
    ...
    successful = verificator->verify(this->getRelatedScene()->getFsm(),&listener);
...
bool FSMSceneView::eventFilter(QObject *watched, QEvent *event) {
