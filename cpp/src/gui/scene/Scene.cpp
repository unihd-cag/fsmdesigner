	...
	if (this->placeMode != CHOOSE) {
		...
		case TRANS: {
			...
			//---------------
                ...
                //Prevent user being able to add second outgoing transition to JoinItem (this->placeTransitionStack.size() == 0 means that the transition has not yet a startpoint)
                if(itemUnder->type() == JoinItem::Type && dynamic_cast<JoinItem*>(itemUnder)->getOutgoingTransition()!=nullptr && this->placeTransitionStack.size() == 0)
                {
                    break;
                }

                //Prevent user being able to connect a transition from Hypertransition to a JoinItem
                if(itemUnder->type() == JoinItem::Type && this->placeTransitionStack.first()->type() == HyperTransition::Type)
                {
                    break;
                }
			...
			if (stateToState || joinToState || stateToJoin) {
				...
				if (stateToState || stateToJoin) {
                    ...
                    //List of all connected transitions
                    QList<Transline*> connectedTransitions;
                    connectedTransitions.append(lastTransline);

                    //Check if there are trackpoints
                    QGraphicsItem * currentItem = lastTransline->getStartItem();

                    while(currentItem->type() == TrackpointItem::Type)
                    {
                        //Get previous Transline
                        connectedTransitions.append(dynamic_cast<TrackpointItem*> (currentItem)->getPreviousTransline());
                        //Go to previous trackpoint if there is one
                        currentItem = dynamic_cast<TrackpointItem*> (currentItem)->getPreviousTransline()->getStartItem();
                    }
                    //Get middle transition
                    double transAt = ceil(((double)connectedTransitions.size())/2)-1;

                    Transline* textTransline = connectedTransitions.at(transAt);

                    qreal textx = textTransline->getEndItem()->x() -(textTransline->getEndItem()->x() - textTransline->getStartItem()->x())/2;
                    qreal texty = textTransline->getEndItem()->y() -(textTransline->getEndItem()->y() - textTransline->getStartItem()->y())/2;
...
void Scene::keyReleaseEvent(QKeyEvent * keyEvent) {
	...
	if (keyEvent->key() == Qt::Key_Escape && this->getPlaceMode() != CHOOSE) {
        ...
        // At this point, user pressed ESC after placing a HyperTransition but has not yet connected the corresponding transline
        // this->placeTransitionStack.first() is enough because you can't create Trackpoints if HyperTransition was selected, so placeTransitionStack has only one item
        if(this->getPlaceMode() == TRANS && !this->placeTransitionStack.empty() && this->placeTransitionStack.first()->type() == HyperTransition::Type)
        {
            //Get HyperTransition out of placeTransitionStack
            HyperTransition* hyperTrans = dynamic_cast<HyperTransition*>(this->placeTransitionStack.takeFirst());

            // Delete Model
            this->getFsm()->deleteHypertrans(hyperTrans->getModel());

            // Remove from Scene, this method already delets all outgoing transititions
            // because HyperTransition::itemChange(...) is called which deletes all outgoing transitions
            // but we have to clear toPlaceStack too
            this->removeItem(hyperTrans);

            this->toPlaceStack.clear();

            //Finally delete hyperTrans
            if(hyperTrans!=nullptr)
                delete hyperTrans;
        }
        this->setPlaceMode(CHOOSE);
    }
...
void Scene::setPlaceMode(FSMDesigner::Item mode) {
	...
	if (mode==FSMDesigner::CHOOSE) {
        ...
        while (!this->placeTransitionStack.isEmpty()) {
            ...
            //Delete JoinItem if there is one
            else if(FSMGraphicsItem<>::isJoinItem(firstItem)){

                //Get JoinItem out of placeTransitionStack
                JoinItem* joinItem = dynamic_cast<JoinItem*>(firstItem);

                // Delete Model
                this->getFsm()->deleteJoin(joinItem->getModel());

                //Delete JoinItem from Scene
                this->removeItem(joinItem);
    ...
    //--Delete item in list to prevent system crashes (setPlaceMode() is called every time when actions(e.g action_State) are clicked)
     if(this->toPlaceStack.size() > 0)
     {
         //Not for transline
         if(!(FSMGraphicsItem<>::isTransline(this->toPlaceStack.first())))
         {
             this->removeItem(this->toPlaceStack.first());
             this->toPlaceStack.clear();
         }
     }
...
void Scene::manageMouseLeftScene()
{
    //Mouse left scene so change cursor to ArrowCursor
    QApplication::setOverrideCursor(Qt::ArrowCursor);

    //If toPlaceStack has item => hide it
    if(this->toPlaceStack.size() > 0)
    {
        toPlaceStack.first()->setVisible(false);
        toPlaceStack.first()->setEnabled(false);
    }
}

void Scene::manageMouseEnteredScene()
{
    //if in LINKDEPARTURE mode and entering Scene => change cursor to crosscursor
    if(this->placeMode == FSMDesigner::LINKDEPARTURE)
    {
        QApplication::setOverrideCursor(Qt::CrossCursor);
    }
}
