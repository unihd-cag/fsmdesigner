...
<%= puts $eout $::actionName %>::<%= puts $eout $::actionName %>(<%= puts $eout $itemClass %> * object,QUndoCommand * parentCommand,QTreeView * relatedWidget) : ObjectFocusedAction<<%= puts $eout $itemClass %>>(object,parentCommand,relatedWidget) {
