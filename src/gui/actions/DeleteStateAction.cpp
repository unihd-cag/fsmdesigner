...
DeleteStateAction::DeleteStateAction(StateItem * item,QUndoCommand * parentCommand) :ItemFocusedAction<StateItem>(item,parentCommand) {
    ...
    while (!this->item->getIncomingTransitions().isEmpty()) {
        ...
        //--Delete JoinItem that has one or more Trackpoints
        //--At this point all Transitions should be deleted except for JoinItem with Trackpoints if there is one
        else if(t->getStartItem()->type()==TrackpointItem::Type){
            TrackpointItem * trackPoint = FSMGraphicsItem<>::toTrackPoint(t->getStartItem());
            while(FSMGraphicsItem<>::isTrackPoint(trackPoint->getStartItem()))
            {
                //-- Get next Trackpoint
                trackPoint = FSMGraphicsItem<>::toTrackPoint(trackPoint->getStartItem());
            }
            //--If StartItem is JoinItem: delete it
            if(trackPoint->getStartItem()->type()==JoinItem::Type){
                FSMGraphicsItem<>::toJoinItem(trackPoint->getStartItem())->remove(this);
            }
        }
    }
}
