...
void DeleteJoinAction::redo(){
    ...
    //Remove outgoing transition including Trackpoints and further Transitions
    if (this->item->getOutgoingTransition()!=NULL) {

        Transline * nextTransline = this->item->getOutgoingTransition();
        do {
            ...
            if (FSMGraphicsItem<>::isTrackPoint(nextTransline->getEndItem())) {
                ...
                delete nextTransline;
                nextTransline = nullptr;
            }
        } while (nextTransline != NULL);
    }

    //-- Remove JoinItem from GUI
    this->getRelatedScene()->removeItem(this->item);

    //-- Remove Join from Model
