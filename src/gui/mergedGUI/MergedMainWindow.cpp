...
MergedMainWindow::MergedMainWindow() :
    ...
    //---- Disable Undo / Redo
    QWidget * redoWidget = this->MaintoolBar->widgetForAction(action_Redo);
    QWidget * undoWidget = this->MaintoolBar->widgetForAction(action_Undo);
    redoWidget->setEnabled(false);
    undoWidget->setEnabled(false);
...
//  fsm_version += QString::fromStdString(Utils::getPatchVersion());
  ...
  qmb_text_str += QString(" is a advanced tool to create finite<br />"
     "state machines(FSM) in a 	interactive way and translate<br />"
